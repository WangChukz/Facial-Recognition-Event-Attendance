"""
augmentation.py — Face augmentation pipeline for enrollment.
Generates multiple embeddings from a single face image via geometric and photometric transforms.
"""
import cv2
import numpy as np
...
def calibrate_threshold(
    ...
    Args:
        same_user_scores: Similarity scores for same user (high expected)
        diff_user_scores: Similarity scores for different users (low expected)

    Returns:
        (optimal_threshold, accuracy)
    """
    import numpy as np
